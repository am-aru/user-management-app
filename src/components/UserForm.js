...
const UserForm = ({ userToEdit = null, onClose }) => {
  ...
  const {setUsers, userState} = useContext(UserContext);
  ...
  const handleSubmit = (e) => {
    ...
    axios[method](url, formData)
      .then((response) => {
        if (method === 'post') {
          setUsers([...userState.users, response.data]);
        } else {
          setUsers(userState.users.map(user => user.id === userToEdit.id ? response.data : user));
        }
        onClose();
      })
      .catch((err) => {
